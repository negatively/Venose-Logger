...
class RootGUI:
    def __init__(self):
        ...
        self.root.option_add("*Font", "Arial 7")
...
class ComGUI():
    def __init__(self, root, serial, data):
        ...
        self.label_com = Label(
            self.frame, text="Port : ", bg='#fff', width=8, anchor='w')
        self.label_bd = Label(
            self.frame, text='Baud Rate : ', bg='#fff', width=8, anchor='w')
...
class ConnGUI():
    def __init__(self, root, serial, data):
        ...
        self.frame = LabelFrame(root, text="Serial Read Manager", padx = 2, pady=5, bg='#fff',
                                width=60)

        ########## Label ##########        
        self.flush_label = Label(
            self.frame, text="Flush Time : ", bg='#fff', width=10, anchor='w')
        self.sample_label = Label(
            self.frame, text="Sample Time : ", bg='#fff', width=10, anchor='w')
        self.purge_label = Label(
            self.frame, text="Purge Time : ", bg='#fff', width=10, anchor='w')

        ########## Button ##########        
        self.btn_start_stream = Button(
            self.frame, text="Start", state='disabled', width=12, command=self.start_stream)
        self.btn_stop_stream = Button(
            self.frame, text="Stop", state='disabled', width=12, command=self.stop_stream)
        self.btn_save_stream = Button(
            self.frame, text="Save To CSV", state='disabled', width=12, command=self.save_stream)
        ...
        # Static Part #        
        self.canvas = Canvas(self.frame, width = 100, height=100, bg='#fff', highlightthickness=0)  
        self.canvas.create_rectangle(75,75,25,25,outline = '#f11', fill='#fff', width=2)
        self.canvas.create_text(65, 10, anchor=E, text='Temp')
        # Dynamic Part #
        self.text = self.canvas.create_text(64, 60, anchor=E, text='---', font=('Arial 10 bold'))
        # Static Part #
        self.canvas.create_text(55, 65, anchor=E, text='C', font=('Arial 10 bold'))
    ...
    def GraphCtrl(self):
        self.canvas.itemconfig(self.text, text=f"{self.data.StrMsg}")
    
    def ConnGUIOpen(self):
        
        self.frame.grid(row=0, column=4, rowspan=3, columnspan=5, padx=2, pady=5)
        ...
        # self.separator.place(relx=0.452, rely=0, relwidth=0.001, relheight=1)
        self.separator2.place(relx=0.74, rely=0, relwidth=0.001, relheight=1)
        ...
        self.canvas.grid(column = 4, row=1, rowspan = 3)
    ...
    def flush_option_menu(self):
        ...
        self.drop_flush.config(width=8)
    ...
    def sample_option_menu(self):
        ...
        self.drop_sample.config(width=8)
    ...
    def purge_option_menu(self):
        ...
        self.drop_purge.config(width=8)
    ...
    def UpdateChart(self):        
        ...
        self.chartMain.ax.set_ylabel('Data Sensor (mV)', fontsize=5)
        self.chartMain.ax.set_xlabel('Time (s)', fontsize=5)
...
class PlotData():
    def __init__(self, root, serial, data):
        ...
        self.serial = serial
        self.data = data
        self.frame = LabelFrame(root, text="Plotting Data", padx = 2, pady = 2, bg='#fff', width = 60)
        self.CreatFig()
    
    def OpenPlot(self):        
        self.frame.grid(padx=2, columnspan = 7,sticky = NW)
        self.chart.get_tk_widget().grid(column=0, row=4, columnspan=1, rowspan=1)
